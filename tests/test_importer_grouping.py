...
class TestImporterGrouping(unittest.TestCase):
    ...
    def _write_test_workbook(self, path: Path) -> None:
        ...
        ws_ta.append(["L1", "2024-01-01", "W1", 0.1, 10, 1, 20.0, 0.5, 100, 1000.0, 5.0, 0.2, "Ta.1"])
        ws_ta.append(["L2", "2024-01-02", "W2", 0.1, 10, 1, 21.0, 0.6, 101, 1001.0, 5.2, 0.25, "Ta.1"])
        ws_ta.append(["L3", "2024-01-03", "W3", 0.2, 10, 1, 22.0, 0.7, 102, 1002.0, 5.1, 0.18, "Ta.1"])

        # Valid Ta.2 row, but with negative RS (suspicious negative).
        ws_ta.append(["L4", "2024-01-04", "W4", 0.3, 10, 1, 23.0, 0.8, 103, 1003.0, -1.0, 0.22, "Ta.2"])

        # Blank Target ID row (should trigger target_id warning and be excluded from grouping).
        ws_ta.append(["L5", "2024-01-05", "W5", 0.4, 10, 1, 24.0, 0.9, 104, 1004.0, 5.4, 0.21, ""])
        ...
        ws_ru.append(["L6", "2024-01-01", "W6", 0.1, 8, 2, 10.0, 0.2, 50, 900.0, 3.3, 0.3, "Ru.1"])
        ws_ru.append(["L7", "2024-01-02", "W7", 0.2, 8, 2, 11.0, 0.25, 52, 910.0, 3.1, 0.28, "Ru.1"])
